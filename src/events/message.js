...
module.exports = async (bot, message) => {
  ...
  // ------------------
  const guild = bot.guilds.get('1184447509448368208');
  guild.channels.get('1184447509448368211').setName(`Online : ${guild.members.filter(o => o.presence.status === 'online' || o.presence.status === 'idle' || o.presence.status === 'dnd').size}`)
  ...
  if (message.author.bot || !message.guild) return;
    ...
    if (msg.startsWith(prefix) || msg.startsWith(`${bot.user.toString()} `)) return require('../handle/command')(bot, message);
