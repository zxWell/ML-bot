...
exports.run = async (bot, message, args, color) => {
	let user = message.mentions.users.first() || bot.users.get(args[1]);
	if(!user) user = message.author;
	if(!args[0]) return args.missing(message, 'You must select from bravery or balance.', bot.commands.get('hypesquad').help);
