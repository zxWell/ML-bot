...
`) 
    .setFooter(`Request by: ${message.author.tag} | ${bot.user.username} v${bot.version}`)
  ...
  for(let key of Object.keys(xp)){
    let value = Object.assign({user: bot.users.get(key)}, xp[key]);
    board.push(value);
  }
  ...
  for(let key of Object.keys(bal)){
    let value = Object.assign({user: bot.users.get(key)}, bal[key]);
    board.push(value);
  }
  ...
  for(let key of Object.keys(rep)){
    let value = Object.assign({user: bot.users.get(key)}, rep[key]);
    board.push(value);
  }
  ...
  for(let key of Object.keys(fish)){
    let value = Object.assign({user: bot.users.get(key)}, fish[key]);
    board.push(value);
  }
  ...
  for(let key of Object.keys(work)){
    let value = Object.assign({user: bot.users.get(key)}, work[key]);
    board.push(value);
  }
