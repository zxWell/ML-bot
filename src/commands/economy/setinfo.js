...
exports.run = async (bot, message, args, color) => {
  ...
  let len = 115;
  if(!newInfo) return args.missing(message, 'You need to specify the message to set your info box', bot.commands.get('setinfo').help);
  if(newInfo.length > len) return message.channel.send(`**${message.author.username}** *thats to length*, **Max \`115\` Characters Allowed.**`);
  let newsInfo = bot.util.chunk(newInfo, 23).join('\n');
